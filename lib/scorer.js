...
function loadConfig() {
  if (!_config) {
    const p = path.join(getDataDir(), 'score-config.json');
 ...
 *
 * @param {object} cvData - Structured CV data
 * @param {string} [lang='en'] - Language selector: 'en' or 'es'.
 *   Controls which action-verb list is used for bullet-lead detection.
 * @returns {object} Scorer result
 */
function scoreRecruiterAppeal(cvData, lang) {
  const config = loadConfig();
  const recConfig = config.recruiter;
  const thresholds = recConfig.thresholds;
  const subWeights = recConfig.subWeights;
  const maxScore = 100;
  const verbList = lang === 'es' ? recConfig.actionVerbsEs : recConfig.actionVerbs;
  ...
  for (const b of bullets) {
    ...
    if (leadsWithActionVerb(b.text, verbList)) {
      actionVerbCount++;
    } else {
 ...
 * @param {string} [options.lang='en'] - Language selector: 'en' or 'es'.
 *   Controls which action-verb list is used in recruiter-appeal scoring.
...
function scoreCV(cvData, matchResult, options) {
  ...
  const recruiterResult = scoreRecruiterAppeal(cvData, opts.lang || 'en');
