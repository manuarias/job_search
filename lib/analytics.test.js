...
describe('analytics — migrateTracking integration', () => {
  ...
  it('parses real jd-tracking.md into 7 entries', () => {
    const result = migrateTracking(trackingPath);
    expect(result.errors).toHaveLength(0);
    expect(result.tracking).toHaveLength(7);
  ...
  it('extracts correct scores from real data', () => {
    ...
    expect(byRef['ARXX']).toBe(64);
    expect(byRef['UNKN']).toBe(82);
  });

  it('extracts correct statuses', () => {
    const result = migrateTracking(trackingPath);
    const statuses = result.tracking.map(t => t.status);
    expect(statuses.filter(s => s === 'Closed')).toHaveLength(4);
  ...
  it('extracts dates in YYYY-MM-DD format for well-formed entries', () => {
    ...
    // ARXX row has a known column misalignment in the source table;
    // exclude it from date-format assertions.
...
describe('analytics — E2E', () => {
  ...
  it('report contains concrete numbers', () => {
    const content = fs.readFileSync(reportPath, 'utf8');
    expect(content).toContain('81.2'); // mean (7 entries)
    expect(content).toContain('88'); // max
    expect(content).toContain('64'); // min
    expect(content).toContain('82'); // median
  });
});
