...
function renderSkills(skills, hkDetails) {
  ...
  if (hkDetails && ((hkDetails.matched && hkDetails.matched.length > 0) || (hkDetails.missed && hkDetails.missed.length > 0))) {
    const jdKeywords = [
      ...(hkDetails.matched || []),
      ...(hkDetails.missed || []),
    ];
 ...
 * Earlier career entries are handled exclusively by `renderExtraSections()`.
 *
 ...
 *
 * Certifications and languages are handled exclusively by `renderExtraSections()`.
 *
 * @param {Array<{degree: string, institution: string, year?: string}>} edu - Education entries.
 * @param {Array<{name: string, issuer?: string, year?: string}>} [certs] - Deprecated. Use renderExtraSections instead.
 * @param {Array<{language: string, proficiency: string}>} [langs] - Deprecated. Use renderExtraSections instead.
...
function renderEducation(edu, certs, langs) {
  ...
  if (edu && edu.length > 0) {
    const eduItems = edu
      .map(e => {
        const line = `${escapeHtml(e.degree)} \u2014 ${escapeHtml(e.institution)}${e.year ? ' (' + escapeHtml(e.year) + ')' : ''}`;
        return `  <li>${line}</li>`;
      })
      .join('\n');
    parts.push(`<ul>\n${eduItems}\n</ul>`);
