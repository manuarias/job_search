...
function loadCV(lang) {
  // All language variants use the same anonymous fixture — render functions are language-agnostic
  return JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'test-fixtures', 'anonymous-cv.json'), 'utf8'));
}
...
describe('renderHeader()', () => {
  ...
  test('renders full header with name, titles, and contact', () => {
    ...
    expect(html).toContain('New York, NY, USA');
    expect(html).toContain('jane.smith@example.com');
    expect(html).toContain('<a href="mailto:jane.smith@example.com">');
    expect(html).toContain('<a href="https://www.linkedin.com/in/janesmith">LinkedIn</a>');
  });
...
describe('renderExperience()', () => {
  ...
  test('renders all primary experience entries', () => {
    ...
    expect(html).toContain('InnoSoft');
  });

  test('limits achievements per role to default 4 (Scenario 2.2)', () => {
    // TechCorp Project Leader has 4 achievements — all should be included
    const html = renderExperience(cv.professionalExperience);
...
describe('Language variants (ES)', () => {
  ...
  test('renderSummary renders summary narrative text', () => {
    const html = renderSummary(cvES.professionalSummary);
    expect(html).toContain('10+ years'); // English text from fixture
    expect(html).not.toContain('<script>');
  });

  test('renderExperience renders achievements', () => {
