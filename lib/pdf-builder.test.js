...
describe('renderExperience()', () => {
  ...
  test('renders earlier experience section', () => {
    const html = renderExperience(cv.professionalExperience, cv.earlierExperience);
    expect(html).toContain('earlier-experience');
    expect(html).toContain('Earlier Career');
    expect(html).toContain('Web Developer');
    expect(html).toContain('Fundación Conocimiento Abierto');
  });
...
describe('renderEducation()', () => {
  ...
  test('renders languages when provided', () => {
  ...
  test('renders certification without issuer', () => {
    const html = renderEducation([], [{ name: 'PMP' }]);
...
describe('Language variants (ES)', () => {
  ...
  test('renderExperience renders Spanish achievements', () => {
    const html = renderExperience(cvES.professionalExperience, cvES.earlierExperience);
    expect(html).toContain('Mercado Libre');
    expect(html).toContain('Project Leader');
    expect(html).toContain('earlier-experience');
  });
...
describe('Integration with real CV data', () => {
  ...
  test('all renderers produce non-empty output with real data', () => {
    ...
    expect(renderEducation(cv.education, cv.certifications, cv.languages).length).toBeGreaterThan(20);
  });
  ...
  test('all renderers escape user content (no raw HTML injection possible)', () => {
    ...
    const exp = renderExperience(cv.professionalExperience, cv.earlierExperience, matchResult);
    const edu = renderEducation(cv.education, cv.certifications, cv.languages);
