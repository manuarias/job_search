...
// ── data file paths ────────────────────────────────────────────────────────
const SOFT_SYNONYMS_PATH = path.join(__dirname, '..', 'data', 'soft-synonyms.json');
const DOMAIN_MAPPING_PATH = path.join(__dirname, '..', 'data', 'domain-mapping.json');
const MATCH_WEIGHTS_PATH = path.join(__dirname, '..', 'data', 'match-weights.json');
...
function loadSynonyms() {
  if (!_synonyms) _synonyms = require(SOFT_SYNONYMS_PATH);
  return _synonyms;
}

function loadDomainMapping() {
  if (!_domainMapping) _domainMapping = require(DOMAIN_MAPPING_PATH);
  return _domainMapping;
}

function loadMatchWeights() {
  if (!_matchWeights) _matchWeights = require(MATCH_WEIGHTS_PATH);
