/**
 * Hermes — JD-to-CV Pipeline Orchestrator (Library)
 *
 * Programmatic API for the full CV optimization pipeline: scrape → extract →
 * match → score → assemble → cover letter. File-system-based state survives
 * crashes and enables resume.
 *
 * Usage:
 *   const { runPipeline } = require('./lib/hermes');
 *   const result = await runPipeline('https://boards.greenhouse.io/...');
 *
 * CJS module — consumed via require().
 */
...
// readline is lazy — only required inside runPipeline when interactive

// ── Library imports ───────────────────────────────────────────────────────
const { scrapeJD, generateRef } = require('./jd-scraper');
const { extractKeywords } = require('./keyword-extractor');
const { matchCV } = require('./matcher');
const { scoreCV } = require('./scorer');
const { assembleCV } = require('./assembler');
const { generateCoverLetter } = require('./cover-letter');
const { generateReport } = require('./reporter');

// ── Paths ─────────────────────────────────────────────────────────────────
const { getDataDir, getApplicationsDir } = require('./data-paths');

const PROJECT_ROOT = path.resolve(__dirname, '..');
const APPS_DIR = getApplicationsDir();
...
function appendTrackingMd(ref, company, role, lang) {
  // If tracking file doesn't exist (fresh clone), copy from template
  if (!fs.existsSync(TRACKING_MD)) {
    const templatePath = path.join(APPS_DIR, 'jd-tracking.md.template');
    if (fs.existsSync(templatePath)) {
      fs.copyFileSync(templatePath, TRACKING_MD);
    } else {
      // Create bare minimum tracking file
      fs.writeFileSync(TRACKING_MD, '# Job Description — CV Tracking\n\n'
        + '## Postulaciones\n\n'
  ...
  const row = `| ${ref} | ${company} | ${role} | [JD](${ref}/job-description.md) | [CV](${ref}/arias_emanuel-${cvLang}-${ref}.md) | — | In Progress | ${today} | ${today} |`;
...
function updateTrackingJson(ref, company, role, score, status) {
  const trackingPath = path.join(getDataDir(), 'jd-tracking.json');
  let tracking;
  try {
...
async function runPipeline(input, opts = {}) {
  ...
  appendTrackingMd(ref, scrapeResult.company, scrapeResult.title, lang);
  ...
  // Capture step results for structured output
  let matchResult = null;
  ...
  for (const s of steps) {
    ...
    if (opts.interactive && rl) {
      ...
      if (answer === 'quit') {
        ...
        if (rl) rl.close();
        return {
          ref, company: scrapeResult.company, role: scrapeResult.title, lang, dir,
          status: 'paused',
          score: null, gap: null, categories: null,
          matchLevel: null, recommendation: null, keywordCoverage: null,
          reportCard: null, report: null,
          files: fs.existsSync(dir) ? fs.readdirSync(dir).filter(f => !f.startsWith('.')).sort() : [],
          state,
        };
      }
      if (answer === false) {
    ...
    try {
      const stepResult = s.fn();
      if (s.name === 'match') matchResult = stepResult;
      if (s.name === 'score') scoreResult = stepResult;
      stepDone(state, s.name);
      saveState(dir, state);
    } catch (stepError) {
      console.error(`\n❌ Pipeline error in step '${s.name}': ${stepError.message}`);
      if (rl) rl.close();

      // Build partial result with whatever we have so far
      const partialFiles = fs.existsSync(dir) ? fs.readdirSync(dir).filter(f => !f.startsWith('.')).sort() : [];

      return {
        ref, company: scrapeResult.company, role: scrapeResult.title, lang, dir,
        status: 'error',
        error: stepError.message,
        score: scoreResult ? scoreResult.final : null,
        gap: scoreResult ? (scoreResult.gapToTarget ? scoreResult.gapToTarget.gap : null) : null,
        categories: scoreResult ? scoreResult.categories : null,
        matchLevel: matchResult ? (matchResult.recommendation ? matchResult.recommendation.level : null) : null,
        recommendation: matchResult ? (matchResult.recommendation && matchResult.recommendation.actions && matchResult.recommendation.actions.length > 0 ? matchResult.recommendation.actions[0].action : null) : null,
        keywordCoverage: matchResult ? (matchResult.summary && matchResult.summary.keywordCoverage ? matchResult.summary.keywordCoverage.percentage : null) : null,
        reportCard: null,
        report: null,
        files: partialFiles,
        state,
  ...
  // Resume fallback: read from disk if results weren't captured in the loop
  if (!scoreResult) {
    try {
      const scorePath = path.join(dir, 'score.json');
      scoreResult = JSON.parse(fs.readFileSync(scorePath, 'utf8'));
    } catch { /* score.json may not exist */ }
  }
  if (!matchResult) {
    try {
      const matchPath = path.join(dir, 'match.json');
      matchResult = JSON.parse(fs.readFileSync(matchPath, 'utf8'));
    } catch { /* match.json may not exist */ }
  }
  ...
  let reportText = null;
  try {
    reportText = generateReport(state);
