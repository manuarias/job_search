 ...
 *
 * Tests cover:
 *   1. English JD fixture (with MUST HAVES section)
 *   2. Spanish JD fixture (with Conocimientos técnicos requeridos)
...
function loadFixture(name) {
  const fileMap = {
    'EN': 'anonymous-jd-en.md',
    'ES': 'anonymous-jd-es.md',
  };
...
describe('keyword-extractor — EN JD', () => {
  let result;
  let jdText;

  beforeAll(() => {
    jdText = loadFixture('EN');
...
describe('keyword-extractor — ES JD', () => {
  let result;

  beforeAll(() => {
    const jdText = loadFixture('ES');
    result = extractKeywords(jdText);
  });
  ...
  it('extracts general tech terms from Spanish JD', () => {
    // The JD mentions general tech terms that should be picked up by the taxonomy
    const terms = result.hardKeywords.map(k => k.term);
    expect(terms.length).toBeGreaterThan(0);
  });

  it('detects REST API from the Spanish JD', () => {
    // Spanish JD mentions "APIs REST" — should match variant mapping "REST" → "REST API"
    // or "API" → "REST API"
    ...
    // The JD mentions REST APIs in the requirements section
  ...
  it('detects SQL', () => {
    // Spanish JD mentions SQL in the requirements section
...
describe('keyword-extractor — integration with real JDs', () => {
  it('EN JD: must-have keywords are correctly flagged', () => {
    const jdText = loadFixture('EN');
    const result = extractKeywords(jdText);

    // EN JD has explicit "MUST HAVES" section
  ...
  it('EN JD: detects seniority correctly (5+ years, Mid-Senior)', () => {
    const jdText = loadFixture('EN');
    const result = extractKeywords(jdText);

    expect(result.senioritySignals.yearsMinimum).toBe(5);
    // Anonymous JD has "Mid-Senior level" → maps to mid
    expect(result.senioritySignals.level).toBe('mid');
  });

  it('EN JD: detects soft skills (communication, stakeholder, cross-functional)', () => {
    const jdText = loadFixture('EN');
    const result = extractKeywords(jdText);
  ...
  it('ES JD: detects Spanish must-have section', () => {
    const jdText = loadFixture('ES');
    const result = extractKeywords(jdText);

    // ES JD has "Conocimientos técnicos requeridos" section
    expect(result.hardKeywords.length).toBeGreaterThan(0);
  });

  it('ES JD: detects semi-senior level from "Nivel esperado"', () => {
    const jdText = loadFixture('ES');
    const result = extractKeywords(jdText);

    // ES JD says "Perfil semi-senior"
    expect(result.senioritySignals.level).toBe('mid');
  });

  it('ES JD: detects soft skills in Spanish', () => {
    const jdText = loadFixture('ES');
    const result = extractKeywords(jdText);
